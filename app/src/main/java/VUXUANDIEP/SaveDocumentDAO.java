...
public class SaveDocumentDAO {
    private DBHelper dbHelper;

    SQLiteDatabase db;

    public SaveDocumentDAO(Context context){
        dbHelper = new DBHelper(context);
        db = dbHelper.getWritableDatabase();
    }

    public long insert(SaveDocumentObject item){
    ...
    public long update(int id, SaveDocumentObject item){
        ContentValues contentValues = new ContentValues();
        contentValues.put("maHP",item.getMaHP());
        contentValues.put("monHoc",item.getMonHoc());
        contentValues.put("ngayLuu",item.getNgayLuu());
    ...
    public ArrayList<SaveDocumentObject> getAll(){
        String sql="SELECT * FROM tblLuuTruTaiLieu";
        return (ArrayList<SaveDocumentObject>) getData(sql);
    }

    public SaveDocumentObject getID(String id){
        String sql = "SELECT * FROM tblLuuTruTaiLieu WHERE ID=?";
        List<SaveDocumentObject> list = getData(sql,id);
        return list.get(0);
    }

    @SuppressLint("Range")
    private List<SaveDocumentObject> getData(String sql, String...selectionArgs) {

        List<SaveDocumentObject> list = new ArrayList<>();
        Cursor c = db.rawQuery(sql,selectionArgs);
        while (c.moveToNext()){
            SaveDocumentObject obj = new  SaveDocumentObject();
